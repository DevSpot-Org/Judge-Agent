...
const ProjectChallengeSchema = z.object({
  ...
  hackathon_challenges: z
    .any()
    .describe("Details of the hackathon challenge(s)"),
});
...
export const SubmissionSchema = z.object({
  ...
  hackathon_id: z
    .number()
    .int()
    .describe("ID of the hackathon this project belongs to"),
  name: z.string().min(1).describe("Name of the project"),
  description: z.string().min(1).describe("Full description of the project"),
  tagline: z.string().min(1).describe("Short tagline for the project"),
  ...
  project_url: z
    ...
    .refine(isGithubRepo, {
      message:
        "Source code URL must be a GitHub repository, not an organization",
    })
  ...
  project_challenges: z
    .array(ProjectChallengeSchema)
    .min(1, { message: "Must have at least one challenges" })
    .describe("List of associated project challenges; at least one required"),
});
